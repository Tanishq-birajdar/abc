...
int main() {
    ...
    tf::Task removeTask = taskflow.for_each(wordsToRemove.begin(), wordsToRemove.end(), [&](const std::string& word) {
        removeWord(root, word);
    });

    // Create a parallel for loop to search for some words in the trie data structure
    std::vector<std::string> wordsToSearch = {"hello", "task", "world", "flow"};
    std::vector<bool> searchResults(wordsToSearch.size(), false);
    tf::Task searchTask = taskflow.for_each_index(0, wordsToSearch.size(), [&](std::size_t i) {
